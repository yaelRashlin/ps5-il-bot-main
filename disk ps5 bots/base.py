...
class selenium_init():
    def __init__(self, shop_name) -> None:
        ...
        if self.config.get("general", 'send_email') == "on":
    ...
    def play_sound(self):
        ...
        # playsound.playsound(os.path.join(self.script_dir, self.config.get("general", 'mp3') ), True)  
    ...
    def send_email(msg = None, shop_name = None):
        ...
        receivers = self.config.get("email", 'receivers').split(",")
        server = smtplib.SMTP_SSL('smtp.gmail.com', 465)
        server.login(sender, self.config.get("email", 'token'))
